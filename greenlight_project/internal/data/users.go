...
var ErrDuplicateEmail = errors.New("Deplicate email address")
...
func ValidateEmail(v *validator.Validator, email string) {
	v.Check(email != "", "email", "must not empty")
...
func ValidatePasswordPlaintext(v *validator.Validator, plaintext string) {
	v.Check(plaintext != "", "password", "password must not be empty")
	v.Check(len(plaintext) >= 8, "password", "password must be at least 8 characters long")
	v.Check(len(plaintext) <= 72, "password", "password must not exceed 72 characters")
}

func ValidateUser(v *validator.Validator, user *User) {
	v.Check(user.Name != "", "username", "username must not be empty")
	v.Check(len(user.Name) <= 500, "username", "username must not exceed 500 characters")
	...
	if user.Password.hash == nil {
		panic("missing password hash for current user")
	}
}
...
func (m UserModel) Insert(user *User) error {
	...
	err := m.DB.QueryRowContext(ctx, query, args...).Scan(&user.ID, &user.Create_at, &user.Version)
	if err != nil {
		switch {
